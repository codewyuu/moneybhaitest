...
export default function Dither({
  ...
  const effectiveBaseColor: [number, number, number] = baseColor
    ? baseColor
    : isDark
      // deep near-black for dark mode to keep contrast
      ? [0.05, 0.06, 0.08]
      // soft near-white for light mode to prevent dark patches
      : [0.95, 0.96, 0.98];

  const effectiveWaveColor: [number, number, number] = waveColor
    ? waveColor
    : isDark
      ? [0.45, 0.45, 0.48]
      : [0.88, 0.88, 0.90];

  const effectiveColorNum = isDark ? Math.max(4, colorNum) : Math.max(6, colorNum);
  const effectivePixelSize = isDark ? pixelSize : Math.max(3, pixelSize);
  const effectiveWaveAmplitude = isDark ? waveAmplitude : Math.min(waveAmplitude, 0.22);
  const effectiveWaveFrequency = isDark ? waveFrequency : Math.min(waveFrequency, 2.8);
